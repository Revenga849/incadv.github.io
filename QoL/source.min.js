...
function prestigeCustom(layer) {\n\
	autoPrestige(layer);\n\
	prestiged = true;\n\
	addCombatLog(9, "<span style=\\"font-weight: 600; color:#FF0000;\\">QoL</span> <span style=\\"font-weight: 600;\\">Auto-Prestige Layer " + (layer + 1) + "</span>");\n\
...
function checkStatUpgrades(layer = 0, logType = 9) {\n\
	...
	for (let i = layer; i >= 0; i--) {\n\
		if (typeof game.prestige !== "undefined" && typeof game.prestige[i] !== "undefined") {\n\
			...
			for (let stat = 1; stat <= 3; stat+=2) {\n\
				...
				if (upgradePoints.gte(1)) {\n\
					...
					if (totalNewPoints.gte(new Decimal("ee5")) && newStatPoints.gte(new Decimal("ee5"))) {\n\
						if (game.prestige[i][0].lt(newStatPoints))\n\
							continue;\n\
							\n\
						if (oldStats.eq_tolerance(newStats, "1e-5"))\n\
							continue;\n\
\n\
...
\n\
var bConstraint = true;\n\
function toggleConstraint () {\n\
	bConstraint = !bConstraint;\n\
	var l = document.getElementById("p-settings-constraint-button");\n\
	l.innerText = "Prestige Constraint: ";\n\
	\n\
	var l = document.getElementById("p-settings-constraint-input");\n\
	l.style.display = bConstraint?"inline":"none";\n\
}\n\
\n\
...
function savePreferences() {\n\
	preferences = [];\n\
	preferences.showAuto = bShowAuto;\n\
	preferences.constraint = bConstraint;\n\
...
function loadPreferences() {\n\
	if (preferences.length != 0) {\n\
		bShowAuto = preference.showAuto;\n\
		bConstraint = preferences.constraint;\n\
		format = preferences.format;\n\
		document.querySelectorAll("[id^=ratio]").forEach((a,b)=>{ a.value = preferences.ratios[b];});\n\
		document.querySelectorAll("[id^=cb]").forEach((a,b)=>{ a.checked = preferences.checkboxes[b];});\n\
	}\n\
}\n\
\n\
	...
	for (var i = 0; i < prestigePoints.length; i++) {\n\
		if (game.prestige != undefined) {\n\
			if (game.prestige[i] != undefined) {\n\
				var cur = game.prestige[i][1].plus(1).multiply(i>0?i:1);\n\
                \n\
                ...
                if (getStatUpgradesFromPrestigePoints(upgradePoints).cmp(getStatUpgradesFromPrestigePoints(pstats.add(1)))>=0) {\n\
                    var mult = getStatUpgradesFromPrestigePoints(upgradePoints)\n\
                        .plus(1)\n\
                        .divideBy(pstats.plus(1));\n\
                }\n\
            } else {\n\
                var cur = new Decimal(1);\n\
                var mult = new Decimal(i==0?1:i);\n\
		...
		\n\
		curvals[i] = cur;\n\
		setStatsText(i, formatValue(cur));\n\
		multvals[i] = mult;\n\
		setButtonText(i, formatValue(mult));\n\
		\n\
		if (typeof game.prestige !== "undefined") {\n\
...
\n\
	...
	if (isFloorChanged) {\n\
		...
		if (prestigeLayer >= 0) {\n\
			...
			clearPrevMults(prestigeLayer);\n\
			\n\
			if (prestigeLayer+1 < prestigePoints.length) {\n\
				prevmult[prestigeLayer+1] = multvals[prestigeLayer+1];\n\
			}\n\
		} else {\n\
