var sourcecode = 'function createElement(type, id, className) {\n\
...
var addLog = function(id, text) {\n\
	...
	if (id == 9) {\n\
		...
		if (!bLogPL1 && pl == "1") return;\n\
		if (!bLogPL2 && pl == "2") return;\n\
		if (!bLogPL3 && pl == "3") return;\n\
		\n\
...
\n\
	...
	qoldiv.style.backgroundColor = bgColor;\n\
		...
		divPrestige.style.height = "21.5vw";\n\
...
\n\
	...
	\n\
		var ascrate = createTotalsStat("ascrate", "AP/hour");\n\
		ascrate.style.borderTop = "1px solid " + lineColor;\n\
	qolTotals.append(ascrate);\n\
	\n\
	...
	\n\
		var asctime = createTotalsStat("asctime", "Ascension Time");\n\
		asctime.style.borderTop = "1px solid " + lineColor;\n\
	qolTotals.append(asctime);\n\
	\n\
...
function redraw() {\n\
	...
	try {\n\
		...
		var layers = document.getElementById("qol-prestige");\n\
		var prestigePoints = getAvailablePrestigePoints();\n\
		if (__init) {\n\
			...
			autoSettingsContainer.style.borderBottom = "1px solid " + lineColor;\n\
			\n\
				var LogPL1Toggle = createOptionsButton("p-pl1-toggle-button", "button prestige-menu-button");\n\
				LogPL1Toggle.innerText = "Log PL1 ON";\n\
				LogPL1Toggle.setAttribute("onclick", "toggleLogPL1();");\n\
				LogPL1Toggle.style.display = "none";\n\
			autoSettingsContainer.append(LogPL1Toggle);\n\
			\n\
				var LogPL2Toggle = createOptionsButton("p-pl2-toggle-button", "button prestige-menu-button");\n\
				LogPL2Toggle.innerText = "Log PL2 ON";\n\
				LogPL2Toggle.setAttribute("onclick", "toggleLogPL2();");\n\
				LogPL2Toggle.style.display = "none";\n\
			autoSettingsContainer.append(LogPL2Toggle);\n\
			\n\
				var LogPL3Toggle = createOptionsButton("p-pl3-toggle-button", "button prestige-menu-button");\n\
				LogPL3Toggle.innerText = "Log PL3 ON";\n\
				LogPL3Toggle.setAttribute("onclick", "toggleLogPL3();");\n\
				LogPL3Toggle.style.display = "none";\n\
			autoSettingsContainer.append(LogPL3Toggle);\n\
			\n\
				var prestigeToggle = createOptionsButton("p-prestige-toggle-button", "button prestige-menu-button");\n\
				prestigeToggle.innerText = "Manual Prestige buttons ON";\n\
			...
			\n\
			layers = createElement("div", "qol-prestige", null);\n\
			layers.style.height = "21.5vw";\n\
			layers.style.overflow = "auto";\n\
			layers.style.textAlign = "right";\n\
		...
		\n\
		if (ppl != layers.childElementCount) {\n\
			layers.innerHTML = null;\n\
			if (gameState.prestige != undefined) {\n\
				for (let i = 0; i < ppl; i++) {\n\
					...
					\n\
						var expLabel = createElement("span", "l-" + (i+1), null);\n\
						expLabel.innerHTML = "<b>Layer " + (i+1) + "</b> exp: ";\n\
					layer.append(expLabel);\n\
					\n\
...
function estimateAscLevel() {\n\
	...
	level = Decimal.layeradd(level.pow(1/1.8).mul(100).div(layer.minus(10)).div(getIAPG(layer)),2);\n\
	return level.pow(1.01);\n\
}\n\
\n\
var bLogPL1 = true;\n\
function toggleLogPL1() {\n\
	bLogPL1 = !bLogPL1;\n\
	var l = document.getElementById("p-pl1-toggle-button");\n\
	l.innerText = "Log PL1 " + (bLogPL1?"ON":"OFF");\n\
}\n\
\n\
var bLogPL2 = true;\n\
function toggleLogPL2() {\n\
	bLogPL2 = !bLogPL2;\n\
	var l = document.getElementById("p-pl2-toggle-button");\n\
	l.innerText = "Log PL2 " + (bLogPL2?"ON":"OFF");\n\
}\n\
\n\
var bLogPL3 = true;\n\
function toggleLogPL3() {\n\
	bLogPL3 = !bLogPL3;\n\
	var l = document.getElementById("p-pl3-toggle-button");\n\
	l.innerText = "Log PL3 " + (bLogPL3?"ON":"OFF");\n\
}\n\
\n\
var bPrestige = false;\n\
function togglePrestige() {\n\
	...
	Array.from(document.querySelectorAll("[id^=l][id$=button]")).forEach((a,b)=>{\n\
		...
		a.setAttribute("onclick", bPrestige?"manPrestige(" + b + ");":"");\n\
	});\n\
	var l = document.getElementById("p-prestige-toggle-button");\n\
	l.innerText = "Manual Prestige buttons " + (bPrestige?"ON":"OFF");\n\
}\n\
\n\
var bLog = false;\n\
function toggleLog() {\n\
	...
	Array.from(document.querySelectorAll("[id^=p-pl][id$=button]")).forEach((a,b)=>{\n\
		a.style.display = bLog?"inline":"none";\n\
	});\n\
	if (bLog) {\n\
...
var sTotalsInterval = setInterval(function() {\n\
	...
	if (typeof gameState.prestige !== "undefined") {\n\
		...
		if (typeof gameState.ascension !== "undefined") {\n\
			totals = totals.mul(getAscensionMultiplier());\n\
			//totals.mul(new Decimal(10).pow(gameState.ascension.reduce((a,b,c)=>a.mul(new Decimal(b[1]).add(1).multiply(c>0?c:1).pow(4)), new Decimal(1)).mul(gameState.prestige.length).mul(gameState.prestige.length)));\n\
		}\n\
	}\n\
	\n\
...
function setTotalsText(text) {\n\
	...
	if (typeof gameState.prestige !== "undefined") {\n\
		...
		if (typeof gameState.ascension !== "undefined" && gameState.ascension[0] !== "undefined") {\n\
			...
			document.getElementById("p-ascrate").style.display = "block";\n\
			...
			//new Decimal(10).pow(gameState.ascension.reduce((a,b,c)=>a.mul(new Decimal(b[1]).add(1).multiply(c>0?c:1).pow(4)), new Decimal(1)).mul(gameState.prestige.length).mul(gameState.prestige.length));\n\
			var asctime = ((gameState.tickClock - gameState.ascentionStartTime)/1000);\n\
			var timeToAsc = asctime.toString().toInterval();\n\
			var ascRate = ascensionPoints.div(asctime/3600);\n\
			document.getElementById("l-ascrate-value").innerText = formatValue(ascRate);\n\
			...
			document.getElementById("p-ascrate").style.display = "none";\n\
		...
		//document.getElementById("l-totalsmulti-value").innerText = formatValue(ascTotals.mul(prestigeTotals)) + "x";\n\
