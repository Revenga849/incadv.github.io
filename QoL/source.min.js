...
\n\
	...
	qoldiv.append(divPrestige);\n\
		...
		divAscension.style.height = "21.5vw";\n\
	...
	try {\n\
		...
		if (__init) {\n\
			...
			\n\
				...
				noTabSwitchToggle.innerText = "AL Tab switch ON";\n\
		...
		if (apl != ALs.childElementCount) {\n\
			...
			if (gameState.ascension != undefined) {\n\
				for (let i = 0; i < apl; i++) {\n\
					...
					layer.setAttribute("style", "display: block");\n\
					\n\
						var expLabel = createElement("span", "al-stats-" + (i+1), null);\n\
						expLabel.innerHTML = "<b>Layer " + (i+1) + "</b> stats: ";\n\
					layer.append(expLabel);\n\
					\n\
					...
					layer.append(expValue);\n\
					\n\
						var apLabel = createElement("span", "al-ap" + (i+1), null);\n\
						apLabel.innerHTML = ", <b>AP:</b> ";\n\
					layer.append(apLabel);\n\
					\n\
...
function toggleTabSwitch() {\n\
	...
	l.innerText = "AL Tab switch: " + (bTabSwitch?"ON":"OFF");\n\
}\n\
\n\
var bPrestige = false;\n\
function togglePrestige() {\n\
	...
	Array.from(document.querySelectorAll("[id^=al][id$=button]")).forEach((a,b)=>{\n\
		a.setAttribute("class", bPrestige?"button prestige-menu-button":"");\n\
		a.setAttribute("onclick", bPrestige?"manAscend(" + b + ");":"");\n\
	});\n\
...
function getALMulti(layer, stat) {\n\
	return new Decimal(stat).plus(1).pow(2*(layer+2)).round();\n\
}\n\
\n\
...
function getAMult(ascensionPoints, i) {\n\
	...
	if (gameState.ascension != undefined && gameState.ascension[i] != undefined) {\n\
		...
		if (getStatUpgradesFromAscensionPoints(upgradePoints).gte(getStatUpgradesFromAscensionPoints(astats.add(1)))) {\n\
			mult = getALMulti(i, getStatUpgradesFromAscensionPoints(upgradePoints)).div(getALMulti(i, astats));\n\
		}\n\
	} else {\n\
		var upgradePoints = new Decimal(ascensionPoints[i]);\n\
		if (upgradePoints.gt(0)) {\n\
			mult = getALMulti(i, getStatUpgradesFromAscensionPoints(upgradePoints));\n\
		}\n\
	}\n\
	return mult;\n\
}\n\
	...
	if (bPLAL && gameState.ascension != undefined) {\n\
		...
		for (var i = 0; i < ascensionPoints.length; i++) {\n\
			...
			if (gameState.ascension != null && gameState.ascension[i] != null) {\n\
				ap = ap.plus(gameState.ascension[i][0]).plus(getAscensionPointsFromStatUpgrades(new Decimal(gameState.ascension[i][1])));\n\
			}\n\
			setAStatsText(i, formatValue(curvals[i]), formatValue(ap));\n\
